...
export function entityPath(scene, entity, y, x, finalDir, action = null) {
  ...
  entity.easystar.findPath(entityTileX, entityTileY, tileX, tileY, (path) => {
    if (path === null || path.length <= 1) {
      ...
      moveAlongPath(scene, path, entity, finalDir, action);
    }
  });
...
export function moveAlongPath(scene, path, entity, finalDir, setAction = null) {
  if (path.length <= 1) {
...
export function createClock(scene) {
  ...
  scene.dayText = scene.add.text(0, 7,  `${lang.Text('day')} 1`, {
    fontFamily: 'DefaultFont',
    fontSize: '18px',
    stroke: '#3a3a50',
    strokeThickness: 4,
  ...
  scene.calendar = scene.add.container(70, 0, [scene.clock, calendar, scene.dayText])
    .setDepth(1000)
    .setVisible(false)
